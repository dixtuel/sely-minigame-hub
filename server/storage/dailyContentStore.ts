...
export const DAILY_GAMES = ["echo", "knot", "cut", "shadow", "vaka", "tetris", "spark", "coil", "apex", "lift", "breakline"] as const;
...
export type DailyManifest = { date: string; games: DailyGamePack[]; featuredGameIds: DailyGameId[]; generatedAt: string };
...
/** Pick four visible catalogue cards from the same daily seed package.
 * This intentionally uses the generated seeds directly rather than a client
 * side hash, so the cron manifest remains the single public source of truth.
 */
function featuredGameIds(games: DailyGamePack[]): DailyGameId[] {
  const ordered = DAILY_GAMES.map((gameId) => games.find((game) => game.gameId === gameId)).filter(Boolean) as DailyGamePack[];
  if (!ordered.length) return [];
  const packageSeed = ordered.reduce((total, game) => (total + game.seed) >>> 0, 0);
  const start = packageSeed % ordered.length;
  return Array.from({ length: Math.min(4, ordered.length) }, (_, offset) => ordered[(start + offset) % ordered.length].gameId);
}
...
export function createDailyManifest(date = dayKey()): DailyManifest {
  ...
  return { date, games, featuredGameIds: featuredGameIds(games), generatedAt: new Date().toISOString() };
}
