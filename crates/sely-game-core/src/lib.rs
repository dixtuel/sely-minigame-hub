...
/// Lander Game terrain generator
#[wasm_bindgen]
pub fn wasm_generate_lander_terrain(width: f64, height: f64, seed: u32) -> Result<JsValue, JsValue> {
    let terrain = games::lander::generate_lander_terrain(width, height, seed);
    serde_wasm_bindgen::to_value(&terrain).map_err(|e| JsValue::from_str(&e.to_string()))
}

/// Sokoban Game level parser
#[wasm_bindgen]
pub fn wasm_parse_sokoban_level(level_idx: usize) -> Result<JsValue, JsValue> {
    let level = games::sokoban::parse_sokoban_level(level_idx);
    serde_wasm_bindgen::to_value(&level).map_err(|e| JsValue::from_str(&e.to_string()))
}

/// 2048 Game board slide
#[wasm_bindgen]
pub fn wasm_slide_2048_left(r0: u32, r1: u32, r2: u32, r3: u32) -> Result<JsValue, JsValue> {
    let board = [
        [r0, r1, r2, r3],
        [0, 0, 0, 0],
        [0, 0, 0, 0],
    ...
    let res = games::game2048::slide_2048_board(&board, games::game2048::Direction::Left);
    serde_wasm_bindgen::to_value(&res).map_err(|e| JsValue::from_str(&e.to_string()))
}

/// Deterministic configuration for Coil, Apex, Lift and Breakline.
/// The canvas renderers can fall back to their local implementation when WASM
/// is unavailable, but daily route parameters remain generated by the same Rust core.
#[wasm_bindgen]
pub fn wasm_generate_arcade_config(seed: u32, mastery: u32, game_id: &str) -> Result<JsValue, JsValue> {
    let config = games::new_arcade::generate_arcade_config(seed, mastery, game_id);
    serde_wasm_bindgen::to_value(&config).map_err(|e| JsValue::from_str(&e.to_string()))
}

/// Deterministic multi-stage Breakline layouts. The same pure generator also
/// compiles into the native VDS/serverless Rust crate.
#[wasm_bindgen]
pub fn wasm_generate_breakline_run(seed: u32, mastery: u32) -> Result<JsValue, JsValue> {
    let run = games::breakline::generate_breakline_run(seed, mastery);
    serde_wasm_bindgen::to_value(&run).map_err(|e| JsValue::from_str(&e.to_string()))
}
