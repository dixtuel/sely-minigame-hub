...
import { ArrowUpRight, CircleHelp, Cookie, Gamepad2, History, Menu, ShieldCheck, Sparkles, Trophy, X } from "lucide-react";
...
export default function Home({ locale = "tr", directGameId }: { locale?: SiteLocale; directGameId?: string }) {
  ...
  return <main className="hub-page" lang={locale}>
    ...
    <header className="hub-nav">
      <a className="brand-lockup" href="#top" aria-label="SELY.TR home"><img src="/storage/sely-mark_de9c08a5.png" alt="" /><span>SELY<span className="brand-dot">.</span>TR</span></a>
...
function GameCard({ game, score, mastery, index, locale, onPlay }: { game: GameMeta; score: number; mastery: number; index: number; locale: SiteLocale; onPlay: () => void }) {
  const words = copy[locale];
