...
export default function VakaInterrogation({
  ...
  const [showQuickDossier, setShowQuickDossier] = useState(false);
  ...
  const persistSession = (
    ...
    isSolved?: boolean,
    vText?: string | null,
    updatedFinalInterrogation?: Record<string, boolean>
  ) => {
    try {
      if (typeof window !== "undefined") {
        secureStorage.setJSON(`sely_vaka_session_${vakaCase.id}`, {
          ...
          finalInterrogationMap: updatedFinalInterrogation !== undefined ? updatedFinalInterrogation : finalInterrogationMap,
        });
      }
    } catch {}
  };
...
function cleanInterrogationText(text: string): string {
  ...
  const [exposedMap, setExposedMap] = useState<
    Record<string, { sentenceId: string; clueId: string; explanation?: string; timestamp?: number; consumed?: boolean }>
  >({});
  // A contradiction grants exactly one follow-up message before the culprit
  // confesses. Persist it with the case session so a reload cannot duplicate it.
  const [finalInterrogationMap, setFinalInterrogationMap] = useState<Record<string, boolean>>(() => {
    try {
      if (typeof window !== "undefined") {
        const parsed = secureStorage.getJSON<any>(`sely_vaka_session_${vakaCase.id}`, null);
        return parsed?.finalInterrogationMap && typeof parsed.finalInterrogationMap === "object"
          ? parsed.finalInterrogationMap
          : {};
      }
    } catch {}
    return {};
  ...
  useEffect(() => {
    ...
    try {
      if (typeof window !== "undefined") {
        const parsed = secureStorage.getJSON<any>(`sely_vaka_session_${vakaCase.id}`, null);
        if (parsed) {
          if (parsed.finalInterrogationMap && typeof parsed.finalInterrogationMap === "object") {
            setFinalInterrogationMap(parsed.finalInterrogationMap);
          } else {
            setFinalInterrogationMap({});
          }
  ...
  useEffect(() => {
    // Only scroll the transcript container internally, do NOT scroll the whole viewport/window
  ...
  const dispatchAction = async (
    ...
    try {
      ...
      let isExposed = false;
      const hasFinalQuestion = Boolean(finalInterrogationMap[selectedSuspectId]);
      let expSentenceId: string | undefined;
      let expClueId: string | undefined;
      if (exposedMap && exposedMap[selectedSuspectId]) {
        isExposed = !hasFinalQuestion && !exposedMap[selectedSuspectId].consumed;
      ...
      const res = await interrogateMutation.mutateAsync({
        ...
        isFinalInterrogation: hasFinalQuestion,
      ...
      const nextFinalMap = { ...finalInterrogationMap };
      const exposedEntry = exposedMap[selectedSuspectId];
      if (res.finalInterrogationAvailable) {
        nextFinalMap[selectedSuspectId] = true;
      } else if (hasFinalQuestion || isExposed) {
        delete nextFinalMap[selectedSuspectId];
      }
      setFinalInterrogationMap(nextFinalMap);
      if (exposedEntry && (hasFinalQuestion || isExposed)) {
        const nextExposedMap = {
          ...exposedMap,
          [selectedSuspectId]: {
            ...exposedEntry,
            consumed: hasFinalQuestion || !res.finalInterrogationAvailable,
          },
        };
        setExposedMap(nextExposedMap);
        try {
          if (typeof window !== "undefined") {
            secureStorage.setJSON(`sely_vaka_exposed_${vakaCase.id}`, nextExposedMap);
          }
        } catch {}
      }
      ...
      persistSession(finalMap, updatedStressMap, isConfessed || solved, vText, nextFinalMap);
    } catch {
  ...
  return (
    <div className="vaka-interrogation-desk">
      ...
      {/* Mobil ve Masaüstü Şüpheli ve Vaka Kumanda Barı */}
      <div className="vaka-interrogate-top-actions">
        <span className="vaka-suspects-count-badge">
          🎙️ {isEn ? "PERSONS OF INTEREST:" : "ŞÜPHELİLER & TANIKLAR:"} <b>{visibleSuspects.length}</b>
        </span>
        <div className="vaka-interrogate-quick-links">
          {onOpenDossier && (
            <button
              type="button"
              className="vaka-quick-dossier-btn"
              onClick={onOpenDossier}
              title={isEn ? "Open Case File, Victim and Crime Scene Notes" : "Vaka Dosyası, Kurban ve Olay Yeri Raporunu Aç"}
            >
              📂 {isEn ? "Case Dossier" : "Vaka Dosyası"}
            </button>
          )}
          <button
            type="button"
            className="vaka-quick-indict-btn"
            onClick={() => onOpenVerdict(selectedSuspectId)}
            title={isEn ? "Take suspect to court" : "Şüpheliyi mahkemede suçla"}
          >
            🏛️ {isEn ? "Indict" : "Mahkemede Suçla"}
          </button>
        </div>
      </div>
      ...
      <div className="vaka-pressure-panel">
        ...
        {exposedMap[selectedSuspectId] && (
          <div className="vaka-exposed-badge-banner">
            ...
            {finalInterrogationMap[selectedSuspectId] && !solved && (
              <small>
                {isEn
                  ? "One final question remains. Ask why they did it."
                  : "Bir son sorgu hakkınız kaldı. Neden yaptığını sorun."}
              </small>
            )}
          </div>
        )}
      ...
      <div ref={chatContainerRef} className="vaka-chat-transcript">
