...
use super::rng::{game_salt, Mulberry32};
...
pub fn build_shadow_level_candidate(seed: u32, mastery: u32) -> ShadowLevel {
    ...
    let exit = exit_candidates[index_for(seed, game_salt(b"211"), exit_candidates.len())];
    ...
    let pad_b = if !valid_pads_b.is_empty() {
        valid_pads_b[index_for(seed, game_salt(b"937"), valid_pads_b.len())]
    } else {
        ShadowPoint {
            x: col_a,
