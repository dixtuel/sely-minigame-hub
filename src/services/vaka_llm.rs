...
pub struct VakaInterrogationPromptParams<'a> {
    ...
    /// Server-derived game truth. This is never client-controlled and tells the model how the
    /// deterministic rules classify the current button action without making the model the judge.
    pub game_state_context: &'a str,
...
pub fn build_vaka_interrogation_prompt(params: &VakaInterrogationPromptParams) -> String {
    ...
    let evidence_note = params.presented_clue.map(|c| {
        let relation = if c.contradicts_suspect_id.as_deref() == Some(suspect.id.as_str()) {
            if is_en { "This evidence directly contradicts this suspect's recorded story." } else { "Bu delil bu şüphelinin kayıtlı ifadesiyle doğrudan çelişiyor." }
        } else if c.clears_suspect_id.as_deref() == Some(suspect.id.as_str()) {
            if is_en { "This evidence supports or clears this suspect; it is not proof against them." } else { "Bu delil bu şüpheliyi destekliyor veya aklıyor; aleyhine kanıt değil." }
        } else if is_en {
            "This evidence is part of the case but is not a direct contradiction of this suspect."
        } else {
            "Bu delil vakanın parçası ancak bu şüpheliyle doğrudan çelişmiyor."
        };
        if is_en {
            format!("\nTHE DETECTIVE PLACED THIS EVIDENCE ON THE TABLE:\n- Evidence: {}\n- What it establishes: {}\n- Evidentiary significance: {}\n- Relation to your recorded story: {}", c.label_en, c.detail_en, c.significance_en, relation)
        } else {
            format!("\nDEDEKTİF ÖNÜNE ŞU DELİLİ KOYDU:\n- Delil: {}\n- Delilin ortaya koyduğu gerçek: {}\n- Delilin önemi: {}\n- Kayıtlı ifadenle ilişkisi: {}", c.label, c.detail, c.significance, relation)
        }
    }).unwrap_or_default();

    // Keep the model grounded in the selected case's actual facts. This is server-side
    // context, not a second rules engine: the deterministic engine still decides scoring,
    // stress, contradiction validity, and confession state.
    let case_facts = if is_en {
        let timeline = params.case_data.timeline.iter()
            .map(|e| format!("- {}: {}", e.time, if !e.event_en.is_empty() { &e.event_en } else { &e.event }))
            .collect::<Vec<_>>().join("\n");
        let scene = params.case_data.crime_scene_notes.en.join("\n- ");
        let clues = params.case_data.clues.iter()
            .map(|c| {
                let relation = if c.contradicts_suspect_id.as_deref() == Some(suspect.id.as_str()) { "contradicts this suspect" }
                    else if c.clears_suspect_id.as_deref() == Some(suspect.id.as_str()) { "clears this suspect" }
                    else { "not directly tied to this suspect" };
                format!("- {}: {} [{}; {}]", c.label_en, c.detail_en, c.significance_en, relation)
            }).collect::<Vec<_>>().join("\n");
        format!("\n## PRIVATE CASE FACTS (AUTHORITATIVE; USE ONLY TO KEEP YOUR ROLEPLAY CONSISTENT)\n- Case: {}\n- Briefing: {}\n- Incident: {} at {}\n- Victim and cause: {} — {}\n- Actual method: {}\n- Actual motive: {}\n- Actual culprit identity: {}\nTIMELINE:\n{}\nCRIME-SCENE FACTS:\n- {}\nALL CASE EVIDENCE:\n{}\nDo not reveal this dossier as a list. Answer only in character and only disclose a fact when the detective's question or presented evidence naturally calls for it.", params.case_data.title_en, params.case_data.briefing_en, params.case_data.location_en, params.case_data.incident_time, params.case_data.victim.name, params.case_data.victim.cause_of_death_en, params.case_data.correct_method_en, params.case_data.correct_motive_en, params.case_data.culprit_id, timeline, scene, clues)
    } else {
        let timeline = params.case_data.timeline.iter()
            .map(|e| format!("- {}: {}", e.time, e.event)).collect::<Vec<_>>().join("\n");
        let scene = params.case_data.crime_scene_notes.tr.join("\n- ");
        let clues = params.case_data.clues.iter()
            .map(|c| {
                let relation = if c.contradicts_suspect_id.as_deref() == Some(suspect.id.as_str()) { "bu şüpheliyle çelişiyor" }
                    else if c.clears_suspect_id.as_deref() == Some(suspect.id.as_str()) { "bu şüpheliyi aklıyor" }
                    else { "bu şüpheliyle doğrudan bağlantılı değil" };
                format!("- {}: {} [{}; {}]", c.label, c.detail, c.significance, relation)
            }).collect::<Vec<_>>().join("\n");
        format!("\n## ÖZEL VAKA GERÇEKLERİ (YETKİLİ DOSYA BAĞLAMI; ROLÜ TUTARLI OYNATMAK İÇİN KULLAN)\n- Vaka: {}\n- Briefing: {}\n- Olay: {}, {}\n- Kurban ve neden: {} — {}\n- Gerçek yöntem: {}\n- Gerçek motivasyon: {}\n- Gerçek failin kimliği: {}\nZAMAN ÇİZELGESİ:\n{}\nOLAY YERİ GERÇEKLERİ:\n- {}\nVAKADAKİ TÜM DELİLLER:\n{}\nBu dosyayı liste halinde açığa vurma. Yalnızca karakter olarak ve dedektifin sorusu veya sunduğu delil gerektirdiğinde ilgili gerçeği söyle.", params.case_data.title, params.case_data.briefing, params.case_data.location, params.case_data.incident_time, params.case_data.victim.name, params.case_data.victim.cause_of_death, params.case_data.correct_method, params.case_data.correct_motive, params.case_data.culprit_id, timeline, scene, clues)
    };

    // The UI sends generated detective messages for buttons (evidence, cross-exam, bluff,
    // silence). Keep the action semantics explicit, while all case facts remain data-driven
    // from the selected dossier above.
    let action_note = match params.action_type.unwrap_or("question") {
        "present_evidence" => if is_en {
            "\nACTION CONTEXT: The detective is demanding a direct explanation of the evidence currently on the table."
        } else {
            "\nHAMLE BAĞLAMI: Dedektif şu anda masadaki delil için doğrudan açıklama istiyor."
        },
        "cross_examine" | "confront" => if is_en {
            "\nACTION CONTEXT: The detective is using another person's testimony as a pressure tactic. Address the actual relationship and facts in the dossier."
        } else {
            "\nHAMLE BAĞLAMI: Dedektif başka bir kişinin ifadesini baskı aracı olarak kullanıyor. Dosyadaki gerçek ilişki ve olgulara göre yanıt ver."
        },
        "bluff" => if is_en {
            "\nACTION CONTEXT: The detective has made a bluff. React according to your actual guilt, stress, and the dossier facts; do not invent physical evidence."
        } else {
            "\nHAMLE BAĞLAMI: Dedektif blöf yaptı. Gerçek suçluluk durumuna, stresine ve dosya olgularına göre tepki ver; yeni fiziksel delil uydurma."
        },
        "stay_silent" => if is_en {
            "\nACTION CONTEXT: The detective deliberately remains silent. Respond naturally to the silence according to your character and stress."
        } else {
            "\nHAMLE BAĞLAMI: Dedektif bilerek sessiz kaldı. Karakterine ve stresine göre bu sessizliğe doğal tepki ver."
        },
        _ => "",
    };
    let clue_note = format!("{}{}{}\n{}", case_facts, action_note, evidence_note, params.game_state_context);
...
pub const VAKA_MODEL_CANDIDATES: &[LlmModelSpec] = &[
    ...
    LlmModelSpec { provider: "groq", model: "llama-3.3-70b-versatile", temperature: 0.7, top_p: None, max_tokens: 1536, timeout_ms: 7500, extra: None },
    LlmModelSpec { provider: "nvidia", model: "nvidia/nemotron-3.5-lightning-30b-a3b", temperature: 0.6, top_p: None, max_tokens: 1536, timeout_ms: 8000, extra: None },
    LlmModelSpec { provider: "mistral", model: "mistral-small-latest", temperature: 0.65, top_p: None, max_tokens: 1536, timeout_ms: 8000, extra: None },
    // 2. Kademe: Dengeli
    LlmModelSpec { provider: "nvidia", model: "google/gemma-4-31b-it", temperature: 0.6, top_p: None, max_tokens: 1024, timeout_ms: 8500, extra: None },
    LlmModelSpec { provider: "nvidia", model: "nvidia/nemotron-3-nano-omni-30b-a3b-reasoning", temperature: 0.6, top_p: Some(0.95), max_tokens: 65536, timeout_ms: 9500, extra: Some(r#"{"reasoning_budget":16384}"#) },
    LlmModelSpec { provider: "nvidia", model: "meta/muse-glimmer-30b", temperature: 0.95, top_p: Some(1.0), max_tokens: 2048, timeout_ms: 10500, extra: Some(r#"{"reasoning_effort":"high"}"#) },
...
pub async fn execute_vaka_llm_chain(
    ...
    for spec in candidates {
        ...
        if let Some(extra) = spec.extra.and_then(|raw| serde_json::from_str::<Value>(raw).ok()) {
            if let (Some(body), Some(extra)) = (payload.as_object_mut(), extra.as_object()) {
